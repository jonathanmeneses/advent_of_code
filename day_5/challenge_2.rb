...
updates.each do |update|
  ...
  valid_update?(update, filtered_rules) ? valid_updates << update : invalid_updates << update
end

corrected_updates = []
invalid_updates.each do |update|
  filtered_rules = filter_for_rules(update, rules_by_first_element)
  t_sorted = filtered_rules.TSort
  # filter t_sorted so only values in update are present
end
